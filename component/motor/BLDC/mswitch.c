...
// modify by goldjun
  ...
  {
    Motordata.SWhkey.gear = SPEED_GRADE_MAX;
  }
  Motordata.UI.duty = EVERY_GRADE_PWM_MAX_TAB[Motordata.SWhkey.gear];
// modify by goldjun
...
//set max pwm value	
  Pwm_Max_Value = EVERY_GRADE_PWM_MAX_TAB[Motordata.SWhkey.gear];
  if(Motordata.MFR == DIR_REVS)
  {
    Pwm_Max_Value = EVERY_GRADE_PWM_REVS_MAX_TAB[Motordata.SWhkey.gear];
  }
  //Limit_Curr = LIMIT_CURR_VOLT_ADC;
  Limit_Curr = EVERY_GRADE_LIMIT_CURR_TAB[Motordata.SWhkey.gear];
  if(Motordata.MFR==DIR_REVS)
  {
  ...
  else if(Motordata.prct.Anp < (Limit_Curr-10))
  {
    ...
    {
      Limit_Curr_CntOff = 0;
      if(Motordata.UI.duty < Pwm_Max_Value)
      {
        Motordata.UI.duty ++;
      }
    }
